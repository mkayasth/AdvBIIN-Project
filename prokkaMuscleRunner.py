...
def remove_whitespace(input_file, output_file):
    ...
    # NJ algorithm here needs whitespace to be removed to function. 	
...
def annotate_and_group_genes(input_fasta, output_dir, muscle_executable='muscle'):
    ...
    # works on intermediate folder and deletes it later.
    ...
    # set fasta header name and file name.    
    for gene_idx in range(max(len(genes) for _, genes in strains)):
        ...
        if grouped_genes:
            ...
            # this will clean, align, and overwrite the `.ffn` file with its final form.
